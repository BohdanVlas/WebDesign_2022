async function addsmt(id, title, imgSrc1, imgSrc2, gauge, ammo, length1, length2, weight1, weight2, mag, text, step1, step2, step3, step4, step5, step6, step7, step8, step9, step10) {
    let container = document.getElementById(id)
    let smt = document.createElement('div')
    let smtHTML = `<h1>${title}</h1>
                        <div class="only-info">
                            <div>
                                <img src="./images/gl/gl/${imgSrc1}" alt="">
                                <img src="./images/gl/gl/${imgSrc2}" alt="">
                            </div>
                            <div>
                                <div>
                                ...
                                <div>
                                    <p>${text}<BR><BR>Неповне розбирання гранатомету проводиться для чищення та змащення в наступному порядку:<BR>
...
{
    fetch("http://localhost:3000/grenade-launcher").then(
        (response)=>{
        ...
        smtArray.forEach(element => {
            ...
            addsmt(id, title, imgSrc1, imgSrc2, gauge, ammo, length1, length2, weight1, weight2, mag, text, step1, step2, step3, step4, step5, step6, step7, step8, step9, step10)
        })
    })
}
Get_SMT();